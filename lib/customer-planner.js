...
import { formatDatedPlannerSection, formatRequestDate } from "./request-dates";
...
export const formatPlannerItemForRequest = (item) => {
  ...
  const lines = [
    ...
    `Included date: ${formatRequestDate(date)}`,
    item.type ? `Work type: ${item.type}` : "",
    plan.title ? `Project name: ${plan.title}` : "",
  ].filter(Boolean);

  PLAN_SECTIONS.forEach(([label, key]) => {
    const section = formatDatedPlannerSection(label, plan[key], date);
